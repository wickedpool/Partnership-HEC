    ...
    <?php
        ...
        foreach ( $equips as $key => $equip ) {
            if ($num == 1) echo '<div class="row">'; //First col, so open the row.
                ...
                . checked( in_array( $key, get_query_var( 'equipements' ) ), true, false ) . '> <label for="equipment[' . $key . ']"><p class = "search-font">' . $equip . '</p><img src="images/equipements/'. $key .'.png"/></label> </div>';
            $num++;
